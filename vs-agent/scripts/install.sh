...
RELEASE_BINARY_NAME="snapsec-detector"
LOCAL_BINARY_NAME="snapsec-detector"
...
echo "Installing Snapsec Detector for ${OS} (${ARCH})..."

# 1. Create Config Directory and File
echo "Configuring detector..."
...
# 2. Download Binary
echo "Downloading detector binary from ${DOWNLOAD_URL}..."
sudo curl -L -f "${DOWNLOAD_URL}" -o "/usr/local/bin/${LOCAL_BINARY_NAME}"
sudo chmod +x "/usr/local/bin/${LOCAL_BINARY_NAME}"

# 3. Run Detector (Triggers Auto-Registration and Service Installation)
